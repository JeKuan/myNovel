...
from read_stat.utils import read_stat_once_read
...
def get_blog_list_common_data(request, blogs_all_list):
    paginator = Paginator(blogs_all_list, settings.BLOG_NUMBER_PER_PAGE ) 
    page_num = request.GET.get('page', 1) #獲取頁碼參數(GET請求)
...
def blog_detail(request, blog_pk):
    ...
    read_cookie_key = read_stat_once_read(request, blog)        
   ...
   # context['comments'] = comments.order_by('-comment_time')
    ...
    response = render(request, 'blog/blog_detail.html', context)#將字典返回到'blogdetail.html'
